...
class CRM_Report_Form extends CRM_Core_Form {
    ...
    function addOrderBys( ) {
        ...
        foreach ( $this->_columns as $tableName => $table ) {
            ...
            // Searchable custom fields are indexed, so allowing to order on them is cheap
            if ( array_key_exists('extends', $table) && ! empty($table['extends']) ) {
                foreach ( $table['fields'] as $fieldName => $field ) {
                    if ( !array_key_exists('no_display', $field) ) {
                        $options[$fieldName] = $field['title'];
                    }
            ...
            // Report developer may define any column to order by, so include them, too
            ...
            if ( CRM_Utils_Array::value( 'order_bys', $this->_params ) &&
                ...
                foreach( $this->_params['order_bys'] as $orderBy ){
                    ...
                    if ( ! empty( $orderByField ) ) {
                        $this->_orderBy[] = "{$orderByField['dbAlias']} {$orderBy['order']}";
...
LEFT JOIN civicrm_contact {$field['alias']} ON {$field['alias']}.id = {$this->_aliases[$table]}.{$columnName} ";
    ...
    function isFieldSelected( $prop ) {
        ...
        if ( !$fieldFound && !empty($this->_params['order_bys']) ) {
            foreach( array_keys($prop['fields']) as $fieldAlias ) {
                foreach ( $this->_params['order_bys'] as $orderBy ){
    ...
    function preProcessOrderBy( &$formValues ) {
        // don't run this twice, and only run it if we have values to process
        static $done;
        if ( $done || empty( $formValues ) ){
            return;
        }
        $done = TRUE;
        ...
        $n = 1;
        for ( $i = 1; $i <= 5; $i++ ) {
            if ( ! empty($formValues['order_bys'][$i]['column']) ) {
                $_showHide->addShow('optionField_'. $n);
                $orderBys[$n] = $formValues['order_bys'][$i];
                $n++;
            }
        }
        for ( $i = $n; $i <= 5; $i++ ) {
